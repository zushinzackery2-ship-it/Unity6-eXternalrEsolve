...
{
    if (!IsInited() || !gameObjectNative || !namespaze || !namespaze[0] || !typeName || !typeName[0])
    {
        return 0;
    }

    std::uintptr_t pool = 0;
    if (!er6::GetComponentPool(Mem(), gameObjectNative, g_ctx.gomOff, pool) || !pool)
    {
        return 0;
    }

    std::int32_t count = 0;
    if (!er6::GetComponentCount(Mem(), gameObjectNative, g_ctx.gomOff, count) || count <= 0)
    {
        return 0;
    }
    ...
    for (std::int32_t i = 0; i < count; ++i)
    {
        ...
        if (!er6::GetComponentSlotNative(Mem(), pool, g_ctx.gomOff, i, nativeComp) || !nativeComp)
        {
            continue;
        }

        std::uintptr_t managedComp = 0;
        if (!er6::GetNativeComponentManaged(Mem(), nativeComp, g_ctx.gomOff, managedComp) || !managedComp)
        {
            continue;
        }

        TypeInfo info;
        if (!er6::ReadManagedObjectTypeInfo(Mem(), managedComp, g_ctx.off, info))
        {
            continue;
        }
...
inline std::uintptr_t GetTransformComponent(std::uintptr_t gameObjectNative)
{
    ...
    {
        const std::uintptr_t mainGo = er6::FindGameObjectThroughTag(5);
        if (mainGo)
        {
            std::uintptr_t pool = 0;
            if (er6::GetComponentPool(Mem(), mainGo, g_ctx.gomOff, pool) && pool)
            {
                std::int32_t count = 0;
                if (er6::GetComponentCount(Mem(), mainGo, g_ctx.gomOff, count) && count > 0)
                {
                    if (count > 1024)
                    {
                        count = 1024;
                    ...
                    for (std::int32_t i = 0; i < count; ++i)
                    {
                        ...
                        if (!er6::GetComponentSlotTypeId(Mem(), pool, g_ctx.gomOff, i, typeId) || typeId == 0)
                        {
                            continue;
                        }
                        ...
                        TypeInfo ti;
                        if (!er6::ReadManagedObjectTypeInfo(Mem(), managedComp, g_ctx.off, ti))
                        {
                            continue;
                        }
    ...
    return er6::GetComponentThroughTypeId(Mem(), g_ctx.gomOff, gameObjectNative, s_transformTypeId);
}
...
inline std::uintptr_t GetCameraComponent(std::uintptr_t gameObjectNative)
{
    ...
    return er6::GetComponentThroughTypeId(Mem(), g_ctx.gomOff, gameObjectNative, s_cameraTypeId);
}
